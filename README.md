...
- **激活码授权**：开启自动抢购前需验证绑定电脑的激活码（一码一机：同一电脑所有浏览器通用，其他电脑无效），带到期时间，到期前无需重复输入。
...
>
> **一码一机**：激活码与电脑绑定。使用方在扩展弹窗复制「本机设备码」（由硬件指纹生成：系统/架构/CPU/内存/分辨率，同一电脑所有浏览器一致）发给你 → 在生成器中填入设备码与到期日期生成激活码 → 该激活码只能在对应电脑上激活（该电脑所有浏览器通用）。换电脑、变更硬件或分辨率后设备码会变，需重新发码。旧版（4 段、无设备码）激活码已全部失效。

### 3. 构建并打包
```bash
# 一条命令完成：清理 → webpack 编译注入脚本 → Next 静态页 → 收尾 → 压 zip
./build-extension.sh                 # 产物 AdzApple_vX.Y.zip
...
3. 点工具栏扩展图标打开弹窗 → 点「确认」开启自动抢购（首次需激活：把弹窗里的「本机设备码」发给发码方换取激活码后输入）。
...
```更新日志
9 — 使用说明补充「手动加购」步骤；品牌更名为「Adzapple助手」；激活码算法升级 SHA-256 + 随机盐。
- v2.

- v3.0 — 激活码升级为机器绑定（一码一机，同一电脑所有浏览器通用）：弹窗展示/复制硬件指纹设备码，生成器需填入设备码，签名纳入设备码；激活状态存 `chrome.storage.local`（不随账号跨设备同步）；旧版 4 段激活码全部失效。- v2.8 — 配置页新增可折叠「使用说明」。
