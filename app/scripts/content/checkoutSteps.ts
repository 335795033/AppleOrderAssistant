...
import { fetchHeaders, defaultAres, CHECKOUT_STEPS, normalizeDistrictName } from '../../shared/constants'
...
/*
 * checkoutx 接口在会话被风控标记/过期时，可能返回 404/403 的 HTML 而不是 JSON。
 * 原代码直接 res.json() 遇到这种情况会抛错，导致整个抢购流程静默中断。
 * 这里统一做状态码 + JSON 解析兜底：解析失败返回 null，由调用方以 { isSuccess: false }
 * 优雅结束本轮，等待下一轮重试（若页面已被整页 404，则由 errorRecover 自动重建会话）。
 */
const parseCheckoutJson = async (result: any): Promise<Record<string, any> | null> => {
    if (!result) return null
    if (![200, 301, 302].includes(Number(result?.status))) {
        console.warn(`checkoutx status error:`, result?.status, result?.url)
        return null
    }
    try {
        return await result.json()
    } catch (e) {
        console.warn(`checkoutx json parse error`, e)
        return null
    }
}
...
const checkoutSteps = async ({ step, x_aos_stk, stepInfo, iPhoneOrderConfig, noNeedTimeSlot }: ICheckoutStepsProps) => {
    ...
    if (noNeedTimeSlot && step == CHECKOUT_STEPS.checkoutFulfillment) {
        ...
        resJson = await parseCheckoutJson(result)
        if (!resJson) return { isSuccess: false }
