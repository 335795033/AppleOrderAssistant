...
const goOrderSteps = async ({ partNumber, x_aos_stk, count, iPhoneOrderConfig }: IGoOrderSteps) => {
    ...
    if (storeNumber) {
        ...
        // 连续轮询多次无货：不直接原地刷新，而是先轮换会话(清除可能已被风控标记的
        // cookie)再刷新，避免长时间同一会话高频刷接口被苹果整页跳到 404
        await rotateSessionAndReload(
            `goOrderSteps failed over ${afterCountThenReload} times, then rotate session and reload page`
        )
    } else {
        // 等待时间叠加 0~3s 随机抖动，让轮询节奏更接近真人，降低被风控识别的概率
        await sleep(
            iPhoneOrderConfig.stepWait + Math.random() * 3,
